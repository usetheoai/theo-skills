...
import { createKeywordRetriever, createStubEmbedder, createVectorRetriever, RetrieverError, stubEmbed , DEFAULT_WORKSPACE_ID } from '@usetheo/skills';
...
describeIntegration('M4 vector + keyword retrievers (T2.2/T2.3)', () => {
  ...
  it('vector retriever orders skills by cosine similarity to the query', async () => {
    ...
    const r = createVectorRetriever({ executor: executor(), embedder: createStubEmbedder() , workspaceId: DEFAULT_WORKSPACE_ID });
  ...
  it('keyword retriever ranks by ts_rank and matches by lexical content', async () => {
    ...
    const r = createKeywordRetriever({ executor: executor() , workspaceId: DEFAULT_WORKSPACE_ID });
  ...
  it('vector retriever throws (typed) on a dimension mismatch, before any SQL', async () => {
    ...
    const r = createVectorRetriever({ executor: executor(), embedder: wrongDim , workspaceId: DEFAULT_WORKSPACE_ID });
  ...
  it('wraps an executor failure in RetrieverError (no raw pg error leak)', async () => {
    ...
    const r = createKeywordRetriever({ executor: boom , workspaceId: DEFAULT_WORKSPACE_ID });
